...
@export_tool_button("Update Project") var update_app_action : Callable = update_app
...
func startup_project() -> void:
	...
	print(" ")
	if not node:
		print("AppTool | Warning! Can't process startup_project function without a valid node:Node propery set!")
		print("AppTool | Cancelling startup_project() call!")
		return
	...
	if project_type == CodeTypes.application: 
		...
		if not FileAccess.file_exists(json_info):
			...
			if not json_tool_node:
				...
				await Make.child(json_tool, node)
